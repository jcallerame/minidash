...
export var multiGroupBy = function multiGroupBy(array, groups) {
  ...
  console.log('group types:', groups.map(function (group) {
    return _typeof(group);
  }));
...
export var equals = function equals(obj1, obj2) {
  for (var p in obj1) {
    if (obj2 == null || !(p in obj2)) {
      console.log("equals(): obj2 == null || ".concat(p, " not in obj2"));
      return false;
    }
    ...
    switch (_typeof(obj1[p])) {
      case 'object':
        if (!equals(obj1[p], obj2[p])) {
          console.log("equals(): Object property ".concat(p, " not equal in the two objects."));
          return false;
        }
      ...
      case 'function':
        if (typeof obj2[p] !== 'function' || obj1[p].toString() != obj2[p].toString()) {
          console.log("equals(): function ".concat(p, " not equal in the two objects."));
          return false;
        }
      ...
      default:
        if (obj1[p] !== obj2[p]) {
          console.log("Regular property ".concat(p, " not equal in the two objects."));
          return false;
        }
  ...
  for (var _p in obj2) {
    if (obj1 == null || !(_p in obj1)) {
      console.log("Property ".concat(_p, " in obj2 but not in obj1."));
      return false;
    }
  }
