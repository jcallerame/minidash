const reSubprops = /^(.*?)\[(['"]?)(.*?)\2\]$/; // Returns the element with the minimum value of the given function called on that element.
...
export const multiGroupBy = (array, groups) => {
  ...
  const groupFuncs = groups.map(group => typeof group === 'string' ? obj => obj[group] : group);
  let result = {};
...
export const isEmpty = obj => {
  return typeof obj === 'object' && Object.keys(obj).length === 0 || typeof obj === 'string' && obj.length === 0;
};
...
export const uniqBy = (list, iteratee) => {
  let set = {};
  let result = [];
  const predicate = typeof iteratee === 'string' ? obj => obj[iteratee] : iteratee;
...
export const sortBy = (list, func, ...moreFuncs) => {
  const objsToSort = list.map(item => ({
    value: item,
    result: func(item)
  }));
  objsToSort.sort((a, b) => {
    ...
    switch (typeof obj1[p]) {
      case 'object':
        if (!equals(obj1[p], obj2[p])) {
          console.log(`equals(): Object property ${p} not equal in the two objects.`);
          return false;
        }
...
export const get = (obj, path, defaultValue) => {
  ...
  for (const elem of path) {
    ...
    if (typeof currObj !== 'object' && typeof currObj !== 'function') throw new Error('get(): Cannot get property of non-object.');
    if (elem == null) throw new Error('get(): Path element is null or undefined.');
